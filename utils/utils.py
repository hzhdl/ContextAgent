...
def compile(solc_version, evm_version, source_code_file):
    ...
    改进：使用 solc_version 参数代替全局 set_solc_version，支持并发编译

    Args:
        solc_version: 编译器版本（如 "0.4.26" 或 "v0.4.26"）
        evm_version: EVM 版本（如 "byzantium", "petersburg"）
        source_code_file: Solidity 源文件路径

    Returns:
        编译输出字典，包含 ABI、bytecode 等
    """
    ...
    try:
        ...
        if version_obj not in installed_versions:
            print(f"Installing solc {version_str}...")
            solcx.install_solc(version_str)

        # 使用 solc_version 参数编译（关键：不调用 set_solc_version）
        ...
        }, allow_paths='.', solc_version=version_obj)
...
def get_functions_from_abi(abi):
    interface_view = []
    interface_update = []
    for field in abi:
        if field['type'] == 'function':
            if field['stateMutability'] in  ['view','pure']:
                ...
                interface_view.append(function_name)
            else:
                function_name = field['name']
                interface_update.append(function_name)
        # elif field['type'] == 'constructor':
            
        #     interface.append('constructor')
    if not "fallback" in interface_update:
        interface_update.append('fallback')
    return [interface_view,interface_update]
...
def get_signature_function_mapping(abi):
    mapping = {}
    for field in abi:
        if field['type'] == 'function':
            function_name = field['name']
...
def convert_stack_value_to_uint(stack_value):
    if stack_value[0] == int:
        return stack_value[1]
    elif stack_value[0] == bytes:
        return int.from_bytes(stack_value[1], "big")
