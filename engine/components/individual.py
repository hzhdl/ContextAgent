...
class Individual():
    def __init__(self, generator):
        ...
        # Store execution result for energy-driven mutation
        self.res_ind = None
    ...
    def init(self, chromosome=None,snapshot_key=None):
        #这里必须输入交易进来
        if not chromosome:
    ...
    def init_constructor(self, chromosome=None,snapshot_key=None):
        #这里必须输入交易进来
        self.chromosome = chromosome
        self.solution = self.decode()
        self.snapshot_key = snapshot_key
    ...
    def set_snapshot_key(self,snapshot_key):
        self.snapshot_key = snapshot_key

    def clone(self):
        indv = self.__class__(generator=self.generator)
        indv.init(chromosome=deepcopy(self.chromosome))
        # Copy res_ind for energy-driven mutation
        indv.res_ind = self.res_ind
        return indv
    ...
    def decode(self):
        ...
        for i in range(len(self.chromosome)):
            ...
            if "mock_call_returns" in self.chromosome[i] and self.chromosome[i]["mock_call_returns"] is not None\
                    and len(self.chromosome[i]["mock_call_returns"]) > 0:
                global_state["mock_call_returns"] = copy(self.chromosome[i]["mock_call_returns"])
